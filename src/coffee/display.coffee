...
Page = React.createClass
  ...
  componentDidMount: () ->
    ...
    socket.on 'login failed', -> alert "Credentials not recognized"

    socket.on 'user creation failed', -> alert 'Username already exists!'
  ...
  toggleFav: (id) ->
    return =>
      if @state.userJWT
        ...
        alert 'Please login to favorite songs!'
  ...
  switchView: ->
    if @state.view == 'tabview'
      @setState view:'signupView'
    else
      @setState view: 'tabview'
